...
export const invoicesApi = {
  ...
  // One row per invoice line, shaped for QuickBooks' own CSV invoice import
  // (Amwai does the club's accounting in QuickBooks).
  exportQuickBooksCsv: (filters: { from?: string; to?: string } = {}) => {
    const p = new URLSearchParams();
    if (filters.from) p.set('from', filters.from);
    if (filters.to) p.set('to', filters.to);
    const s = p.toString();
    return downloadFile(`/invoices/export-quickbooks${s ? `?${s}` : ''}`, 'invoices-quickbooks.csv');
  }
};
